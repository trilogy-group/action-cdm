...
class CdmCommand {
    ...
    async getCdmResult(params = this.getCommandParams()) {
        ...
        const { data } = await axios_default().get(apiEndpoint, {
...
class SchemaNewCommand extends CdmCommand {
    constructor() {
        super();
    ...
    async process() {
        ...
        core.info(`Checking for database existence: ${params.get(CDMCommandParameters.name)}`);
        ...
        try {
            ...
            if (cdmResult === undefined) {
                throw new Error(`Unknown error while checking for database existence: ${error}`);
            }
            let message;
            if (cdmResult.message !== undefined) {
                message = cdmResult.message;
            }
            else if (cdmResult.result !== undefined && cdmResult.result.message !== undefined) {
                message = cdmResult.result.message;
            }
            else {
